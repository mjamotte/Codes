...
if plot_Us==True:
	...
	ax2.semilogx(U_s,E_k_s/N+2*J+E_tr_s/N,'-k',label="$E_{trap}/N+(E_k+2J)/N$")
	...
	fig_OL1D_Us.savefig("Figures_OL_1D_BEC_SC/fig_"+temp+".pdf")
...
if plot_Vs==True:
	...
	list_args = ['N','V','U']
	split = ['N_','V_','U_']
	params_interv = {'U': [1e-4,1.1e-4],'V': np.linspace(1e-6,1e-4,15),'N': [1e4]}
	extension = '.npy'
	files_names = Mf.select_files(path,extension,list_args,params_interv,split)
	...
	fig_OL1D_Vs.savefig("Figures_OL/fig_"+temp+".pdf")